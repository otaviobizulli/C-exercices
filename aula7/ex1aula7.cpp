...
void Ins_Antes_Onde (NO **Inicio, int v, NO *onde)
{
	NO *p, *q;
	p = new NO;
	p->info = v;
...
void Ins_Apos_Onde (int v, NO *onde)
{
	NO *p;
	p = new NO;
	p->info = v;
...
main()
{
	...
	do
	{
		...
		printf("\n06 - Inserir antes do no acessado");
		printf("\n07 - Inserir apos o no acessado");
		printf("\n08 - Acessar");
		printf("\n09 - Listar");
		printf("\n10 - Sair");
		printf("\n\nOpcao => ");
		...
		scanf("%d",&op);
	   	...
	   	switch (op)
	   	{
			...
			case 6: printf("\nValor para acessar => ");
		      		...
		      		onde = Acesso(L,valor);
				    if (onde == NULL)
					 	printf("\nValor nao encontrado!! Impossivel inserir!\n");
				    else
				    {
						printf("\nValor para inserir => ");
					 	fflush(stdin);
					 	scanf("%d",&valor);
					 	Ins_Antes_Onde(&L, valor, onde);
					 	printf("\nInsercao efetuada!!\n");
				    }
				    system("pause");
				    break;
